...
string getView(const string &filename, context &x) {
    return load("/usr/src/cpp_serve/hello_crow/public/" + filename + ".html").render(x);
}

void sendFile(response &res, string filename, string contentType)
{
    ifstream ifs("/usr/src/cpp_serve/hello_crow/public/" + filename, ifstream::in);
...
int main(int argc, char* argv[])
{
    ...
    auto collection = conn["heroku_r5npsqht"]["contacts"];
    ...
    CROW_ROUTE(app, "/contact/<string>")([&collection](string email){
